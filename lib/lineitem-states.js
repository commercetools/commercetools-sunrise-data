...
export const deleteAllStates = () =>
  createStandardDelete({
    itemName: 'transition states',
    service: () =>
      stateService.where('transitions is not empty'),
    ...
    createStandardDelete({
      itemName: 'states',
      service: () => stateService.where('builtIn = false')
    })
  )
...
const createTransactions = (fromId, toIds) =>
  execute({
    ...
    execute({
      ...
      console.log('transaction(s) created for: ' + fromId)
    )
  })
