...
import { setStandaloneVariantMode } from './project-setup'
...
export const importStandaloneProducts = (
  ...
  skipInventory = false,
  inventoryCsv = './data/inventory-standalone.csv'
) => {
  ...
  // First set project to StandaloneVariants mode, then import stores
  return setStandaloneVariantMode()
    .then(() => importStores())
    .then(() =>
      Promise.all(
