...
// return a promise
const createState = ([key, nameEN, nameDE, nameFR, intialState]) =>
  execute({
    uri: stateService.build(),
    method: 'POST',
    body: createStateDraft(key, nameEN, nameDE, nameFR, intialState)
  }).then(({ body: { id } }) => id)
...
export const importProductStates = () =>
  execute({
    ...
    .then((initialId) =>
      Promise.all(
        [
          ['readyForReview', 'Ready for Review', 'Ready for Review', 'Prêt pour contrôle', false],
          ['approved', 'Approved', 'Approved', 'Approuvé', false]
        ]
          .map(createState)
          .concat(initialId)
      )
