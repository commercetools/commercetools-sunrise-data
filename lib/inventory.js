...
if (nconf.get('clean')) {
  ...
  execSync(`${importInventory('stock-import')} --logLevel debug --logSilent true --file data/inventory.csv --projectKey ${process.env.CONFIG_PROJECT_KEY} --clientId ${process.env.CONFIG_CLIENT_ID} --clientSecret ${process.env.CONFIG_CLIENT_SECRET} --sphereHost https://${process.env.CONFIG_API_URL} --sphereAuthHost https://${process.env.CONFIG_AUTH_URL}`)
