...
const deleteAllProducts = (cb) => {
  ...
  client.process(
    ...
    (payload) => {
      ...
      if (results.length > 0) {
        console.log(`Deleting ${results.length} products...`)
      }
      return Promise.all(
        results.map(async (element) => {
          if (element.masterData.published) {
...
if (nconf.get('clean')) {
  ...
  execSync(`${importProducts()} import --matchBy sku --publish --csv data/products.csv -p ${process.env.CTP_PROJECT_KEY} -i ${process.env.CTP_CLIENT_ID}  -s ${process.env.CTP_CLIENT_SECRET} --sphereHost https://${process.env.CTP_API_URL} --sphereAuthHost https://${process.env.CTP_AUTH_URL}`, { stdio: 'inherit' })
}
