...
if (nconf.get('clean')) {
  ...
  execSync(`${importCategories('category-sync')} import -f data/categories.csv -p ${process.env.CTP_PROJECT_KEY} -i ${process.env.CTP_CLIENT_ID}  -s ${process.env.CTP_CLIENT_SECRET} --sphereHost ${process.env.CTP_API_URL} --sphereAuthHost ${process.env.CTP_AUTH_URL}`, { stdio: 'inherit' })
}
