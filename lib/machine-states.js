...
const createStateDraft = (key, name_en, name_de) => {
    return {
        "key": key,
        "type": "LineItemState",
        "initial": false,
...
// return a promise
const createState = (key, name_en, name_de) => {
    return client.execute({
        uri: stateService.build(),
        method: 'POST',
        body: createStateDraft(key, name_en, name_de)
    });
}
...
if(nconf.get("clean")) {
    console.log('about to clean all state machines');
    clearAllStatesWithTransaction();
} else if(nconf.get("import")) {
    console.log('importing states machine items');
    loadInitialStateId();
    AddStateToProject();
}
