...
const createStandaloneVariant = (
  ...
  (product, productId) => {
    const { sku, images, variantKey } = product
    const variant = {
      productId,
      key: variantKey || sku,
    ...
    .then(
      ([
        ...
        return Promise.all(
          ...
          // Then create all standalone variants
          let totalVariants = 0
          const variantPromises = []
          productsWithIds.forEach(({ productId, productType, variants }) => {
...
if (nconf.get('clean')) {
  // Important: Delete standalone variants FIRST, then products, then product types
  // Products can only be deleted after all their variants are deleted
